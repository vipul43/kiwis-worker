...
- Error handlers simplified: always reset to pending status on failure
- Watcher fetches both pending and failed jobs for processing
...
- Separate GetPendingJobs() and GetFailedJobs() repository methods for clarity
- Account sync: processes up to 5 pending + 5 failed jobs per cycle
- Email sync: processes 1 pending job, falls back to 1 failed job if none pending
